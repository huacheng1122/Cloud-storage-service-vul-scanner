...
from manager import *
from base_tools import ConcurrencyModeFlag, check_callable, ArgInfo, select_var_dtype, BaseTool, make_anchor, \
    make_side

RUN_ICON = os.path.join(ICON_PATH, "play_w.png")
LEFT_PAD_LEN = 10
LABEL_WIDTH = 15  # label左侧的宽度
INPUT_BOX_LEN = 70
DEFAULT_PAD = 1
...
class BaseButton(BaseNotebookTool):
    ...
    def build(self, **kwargs) -> tkinter.Frame:
        ...
        self.btn.pack(side=make_side(self.side), padx=self.padx, pady=self.pady)
        return frame
...
class Label(BaseNotebookTool):
    ...
    def build(self, *args, **kwargs) -> tkinter.Frame:
        ...
        label.pack(anchor=make_anchor(self.alignment), padx=0)
        return frame
